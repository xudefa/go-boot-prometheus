# go-boot-prometheus 项目开发规范文档

go-boot-prometheus 是一个基于 [github.com/xudefa/go-boot](https://github.com/xudefa/go-boot) 的 Prometheus 监控集成模块。本模块将 go-boot 指标系统无缝对接到 Prometheus 生态，遵循 go-boot 项目的开发规范。
...
- **集成模块**：go-boot-prometheus 是 go-boot 的监控层集成，将指标系统对接到 Prometheus
- **规范继承**：完全遵循 go-boot 的开发规范、命名约定、代码风格

### 1.2 核心职责

- 将 Prometheus Exporter 注册为 go-boot 容器中的 Bean
- 提供基于 go-boot 自动配置的 Prometheus 启动器
- 实现 `prometheus.Collector` 接口的 BootMetricsCollector
- 支持 Counter、Gauge、Histogram 等指标类型的转换
- 提供 HTTP Handler 暴露 `/metrics` 端点
...
- **监控系统**：集成 Prometheus（`github.com/prometheus/client_golang/prometheus`）
- **职责边界**：仅负责指标转换和暴露，不包含其他业务逻辑
- **示例代码**：统一放在 `examples/` 目录，演示 Prometheus 集成用法

### 2.2 go-boot-prometheus 核心包结构

| 包 | 说明 | 接口定义 |
|---|------|----------|
| `exporter.go` | Prometheus Exporter 实现 | `Exporter`, `BootMetricsCollector` |
| `autoconfig.go` | 自动配置注册 | `AutoConfiguration` |

### 2.3 go-boot 核心包参考

go-boot-prometheus 依赖 go-boot 的以下核心包：
...
go-boot-prometheus 遵循 go-boot 的接口抽象原则，所有集成层通过核心框架中的接口抽象定义，实现运行时互换：
